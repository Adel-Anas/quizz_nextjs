...
function CodeBlock({ code, className = "" }) {
  return (
    <pre className={`overflow-x-auto rounded-lg border border-slate-700 bg-slate-950 p-3 text-xs leading-relaxed ${className}`}>
      <code
        className="font-mono"
...
export default function QuestionCode({ question, onAnswer, selectedAnswer }) {
  return (
    <div className="flex w-full flex-col gap-8">
      ...
      {question.codeSnippet && <CodeBlock code={question.codeSnippet} className="text-sm p-4" />}
        ...
        {question.options.map((option) => {
          ...
          return (
            <button
              ...
              className={`group flex flex-col items-start gap-2 rounded-xl border-2 bg-card p-4 text-left transition-all hover:border-accent/60 hover:bg-slate-800 ${
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <span className="text-sm leading-relaxed text-slate-100 sm:text-base">
                  {option.text}
                </span>
              </div>
              {option.code && <CodeBlock code={option.code} className="w-full" />}
            </button>
          );
        })}
      </div>
